# IFP-21: Declaration Registry

**IFP:** 21
**Title:** Declaration Registry
**Class:** Core
**Status:** Draft
...
This IFP defines the publication format for a **Declaration Registry**: a service that publishes declarations — codes of conduct, professional ethics, philosophies, religious commitments — as versioned, immutable, content-addressed entries, alongside the jurisdiction facts and shared vocabularies that let those entries be cited precisely. It also defines the **citation**, the five-part identifier naming exactly one version of one entry. A registry is a git repository of markdown files with YAML frontmatter, published as a static site with a machine-readable index; the write path is a pull request, and immutability is enforced in CI. Anyone can host a registry. IFP-22 defines the **posture** a principal takes toward a cited declaration.

## Motivation

Citations need somewhere stable to point. The requirements are modest and specific: entries must be resolvable over HTTPS, immutable once published, verifiable against a content hash, mirrorable, and legible to humans in a text editor. A database-backed service would satisfy none of these better than a git repository published statically — and git supplies version history, a review-gated write path, and hosting-anywhere for free.

The registry deliberately does *not* interpret, rank, or endorse what it publishes. It is shelving, not sermon: the SPDX License List holds licenses its maintainers would never choose, and a declaration registry may hold philosophies its operator does not share. Posture-taking belongs to the principals who cite entries (IFP-22), and to registry operators only in their choice of what to shelve.

## 1. Registry Identity

- A registry is identified by its **base URL** (HTTPS), which is the `registry` field of every citation into it.
- "Declaration Registry" is the class name. An **instance name** is the operator's chosen label for one registry, conventionally the leading DNS label or path segment of the base URL — `declarations` in `https://declarations.example.org` — but any label the operator prefers.
- A registry MUST publish a descriptor at:

```
GET <base>/.well-known/ifp/declaration-registry.json
```
...
```json
{
  ...
  "operator": {
    "accountable": "Jane Example",
    "banner": "Example Commons"
  },
...
`operator` names who stands behind the registry. Two genuinely different things want that slot, and conflating them breaks on contact with reality:

- **`accountable`** (REQUIRED) — the natural person or incorporated legal entity responsible for the registry: who holds copyright in the registry's own prose (restatements, translations, vocabularies, the policy page), who a rights claimant writes to, and who can be held harmless. This MUST be a person or a legal entity. An unincorporated project name cannot hold copyright, cannot be held harmless, and gives a claimant nobody to write to.
- **`banner`** (OPTIONAL) — the name the shelf is published under: a collaboration, a working group, a commons, possibly informal. This is frequently the name readers actually know, and it carries no duties.

A registry run by a foundation may set both to the foundation. A registry run by one person under a project's banner sets `accountable` to the person and `banner` to the project. Where this spec assigns a duty to "the operator" — withdrawal (Section 5.1), curation and governance (Section 8), republication rights (Section 8) — that duty lands on `accountable`.

## 2. Citations

A **citation** names exactly one version of one entry in one registry. It is the unit that travels: IFP-22 postures carry citations, and every index row (Section 7) contains one.

A citation is a JSON object with five required fields and one optional (`lang`):

```json
{
  "registry": "https://declarations.example.org",
...
- `name` — the entry's **handle** (Section 3.1): a chosen identifier, unique within the registry. The handle is not the entry's title; per-language titles carry naming (Section 3.2).
- `version` — an opaque string; totally ordered within the entry per the registry's declared ordering scheme (Section 5). Semantic versioning is permitted, never required.
...
- `lang` — OPTIONAL (BCP 47): which language text of the entry the CID pins (Section 3.2). Absent when the entry has a single text.
- `cid` — the IPFS CIDv1 of the cited language text's file, computed per Section 6. The CID makes a citation verifiable offline, survivable if the registry disappears, and mirrorable with proof of fidelity.

A receiver that fetches the cited entry MUST verify the fetched bytes against `cid` before treating the text as the cited entry.

### 2.1 Renderings: translations and restatements

An entry may publish several language texts of one version, each with its own CID, each marked authentic or courtesy (Section 3.2). Citations naming any **authentic** text of the same (registry, name, version) name the same entry. A citation naming a non-authentic translation names the entry *as rendered in that translation*; receivers MAY surface that distinction to their principals rather than assuming equivalence. This is the authentic-texts doctrine of plurilingual legal instruments (Vienna Convention, Article 33; see References) carried over with content addressing doing the work designation clauses do in treaties.

The same rule covers **restatement entries** (Section 3.4), where an instrument that cannot be freely reproduced is restated in the registry's own words: a citation naming a restatement names the entry *as restated* — you cite the text you can read and pin, and receivers MAY surface that it stands one rendering away from the original. Translations and restatements are one phenomenon at the protocol layer: cited renderings. IFP-22, Section 6 states what this means for a posture taken against such a rendering.
...
Frontmatter fields — REQUIRED: `name` (the handle, Section 3.1), `version` (string), `date` (ISO 8601, locked at publication), `entry_type` (Section 3.3), `title` (in this text's own language), `language` (BCP 47 — always declared, never implied), `type` (`full-text` | `restatement`, Section 3.4), `license` (the text must be lawfully copyable **verbatim**: what the CID story needs is byte-exact reproduction for mirroring and pinning, not modification rights, so a `verbatim-only` class — permitting exact-copy reproduction without modification — satisfies the requirement as fully as a free license does). CONDITIONALLY REQUIRED: `category` (Section 4) — REQUIRED when `entry_type: declaration`, otherwise absent. OPTIONAL: `authentic` (Section 3.2), `source_url`, `accessed`, `source_hash`, `supersedes`, `codes` (jurisdiction entries: ISO 3166 codes), `kind` (declarations only — the document's form: creed, oath, code of conduct, and so on; tokens come from a registry `vocabulary` entry, extensible by PR, the same pattern IFP-22's `hosted_on` provider tokens use), `notes`.

### 3.1 Handles

The `name` field is the entry's **handle**: a chosen identifier, not the entry's name. Handles are lowercase ASCII letters, digits, and hyphens; 1–64 characters; no leading or trailing hyphen. This is a constraint on an identifier, not on any language — the handle carries no naming duty, because per-language titles (Section 3.2) are the entry's names.
...
### 3.2 Language texts and authenticity

A version's language texts are parallel renderings, not ordered revisions. Adding a translation to a published version adds a new file and alters none, and is permitted at any time; each published language text is individually write-once (Section 5).

Where a version has several texts, each declares `authentic: true` or `false`. **Authentic** texts are authoritative statements of the entry — several may be equally authentic, in the manner of plurilingual legal instruments — while non-authentic texts are courtesy translations. A version with a single text is authentic by definition (`authentic` may be omitted). Section 2.1 defines what citing each kind means.

Each language text carries its own `title`, in its own language. The titles are the entry's names, and they are plural.

### 3.3 Entry types

A registry shelves three kinds of thing, and they are not interchangeable. `entry_type` is REQUIRED on every language text:

| `entry_type` | What it holds | How it is referenced |
| --- | --- | --- |
| `declaration` | A published text a principal can take a posture toward: a code of conduct, an ethics code, a philosophy, a religious rule of life | Cited in an IFP-22 posture |
| `jurisdiction` | A jurisdiction fact — a legal reality a principal is *subject to*, never in agreement with (codes carried in `codes`) | Referenced by IFP-22 `subject_to`; never carries a posture |
| `vocabulary` | A controlled token list other specs resolve against — hosting-provider slugs, document kinds, display labels for protocol tokens | Referenced by the field whose tokens it defines; never carries a posture |

**The distinction is normative, not editorial.** Only a `declaration` is something a principal can hold an opinion about. Nobody `generally-agrees` with California, and nobody is `bound-by` a list of hosting-provider slugs. IFP-22, Section 3 states the corresponding requirement on the posture side: **a posture's citation MUST name an entry whose `entry_type` is `declaration`.** A registry SHOULD reject, and CI SHOULD flag, an entry that carries `category` or `kind` while declaring a non-`declaration` type — those two fields describe declarations only.

Jurisdiction and vocabulary entries are **apparatus**: they exist so that declarations can be cited, and postures stated, with precision. They are versioned, immutable, and content-addressed on exactly the same terms as declarations, because a token list that shifts underfoot breaks the citations that lean on it.

### 3.4 Full-text and restatement entries
...
- **Citing a restatement means citing the restatement.** Section 2.1 applies: a citation naming a restatement names the declaration *as restated* — the same rule as for courtesy translations, because a restatement is a rendering of the original into the registry's words. Receivers MAY surface the distinction.
- **Preference order.** Where the license allows, publish the authentic full text. Restate where it does not — or where a faithful summary is the honest unit of agreement, since taking a posture toward one read page can mean more than taking one toward forty unread ones.

Restatement entries compose with Section 3.2: a restatement may itself have language texts (the original restatement authentic, its translations courtesy).

## 4. Categories

`category` subdivides **declarations** and applies to no other entry type (Section 3.3). Starter set, extensible by registry policy:
...
| `governance` | Charters, constitutions, bylaws, enforcement procedures |

Earlier drafts listed `jurisdiction` and `vocabulary` here. They are not categories of declaration; they are separate entry types, because a category answers "what kind of declaration is this?" and neither of them is a declaration at all. See Section 3.3.
...
- Versions within one entry MUST be **totally ordered**, and the entry's ordering scheme MUST be stated or inferable per registry policy (semver, integers, and dotted-decimal are all acceptable; semver is never required — a typo-fix to a code of conduct has no meaningful "patch vs. minor" semantics).
- `supersedes` links a version to its predecessor. Retraction is a new version whose body says so; the retracted version remains published, because citations to it remain outstanding — **unless withdrawn** (Section 5.1).
...
- **Error** (wrong, not harmful): supersede promptly (Section 5). The old text stays served; history stays legible. **This is not withdrawal**, and it therefore has no reason class below — an error is corrected by publishing, not by tombstoning.
- **Policy withdrawal** (the operator will no longer shelve it): the entry file is **replaced by a tombstone**. The entry URL answers `410 Gone` with a small document carrying the withdrawn text's identity — `name`, `version`, `language`, and **its CID** — plus the withdrawal date, a reason class (`policy` | `legal` | `mistake`), and a pointer to the policy page. The index row remains, flagged `withdrawn: true`, CID retained. Identity and record survive; serving and shelving stop. Outstanding citations stay interpretable, and cached copies still verify against the CID.
  - The three reason classes: **`policy`** — the operator has decided this does not belong on the shelf. **`legal`** — withdrawal compelled from outside (see the next grade). **`mistake`** — the entry was published in error and has no correct successor to supersede it: the wrong file, a duplicate handle, a text nobody meant to shelve. `mistake` is distinct from the **Error** *grade* above: an Error has a right answer and is fixed by superseding, while a mistaken publication has nothing to supersede *to* and must be un-served. Earlier drafts spelled this class `error`, which read as contradicting the grade taxonomy.
  ...
  "entries": [
    {
      ...
      "type": "full-text",
      "cid": "bafkreib2rxk3rw6vwmlqcjcbxdvcs2wgkxo3uh4rwpldegyzloxbkodo6e",
      "url": "https://declarations.example.org/entries/contributor-covenant/2.1.en.md"
    }
  ]
}
```

Each index row is one language text and contains a complete citation tuple (Section 2) plus the fetch URL, and carries the entry's `entry_type` so a consumer can filter to declarations without fetching each file. A version's translations appear as sibling rows sharing (name, version). A withdrawn text's row remains, flagged `withdrawn: true` with its CID retained (Section 5.1). Human-readable pages per entry are RECOMMENDED; their form is unspecified.

## 8. Write Path and Governance
...
- CI enforces: immutability (Section 5), frontmatter validity, entry-type and category consistency (Section 3.3), slug and size constraints, CID recomputation (Section 6).
- **Curation is the operator's own business, and SHOULD be published as a policy page.** A registry may be ecumenical — a broad shelf of widely-held codes across traditions, deliberately neutral — or it may be opinionated, curated to one community's convictions. Both are conformant. What a registry may not do is *silently* alter what it has published — withdrawal (Section 5.1) is loud, recorded, and identity-preserving.
- **Hosting is not endorsement**, and per IFP-22, a posture of `acknowledges` is not agreement. Registries SHOULD say both of these things on their policy page, because both will otherwise be assumed wrongly.
...
**Write-once rather than editable-with-history.** Citations are the point. An edited entry would silently change what an outstanding IFP-22 posture means; a superseding version changes nothing already declared.
...
- **Tampering.** A registry serving different bytes than a citation's CID is detectable by every verifying receiver (Section 2). Mirrors are held to the same check — fidelity travels with the CID, not the host.
- **Name squatting and confusables.** Slugs are first-come within a registry; operators SHOULD refuse deceptive near-duplicates of existing entry names. Cross-registry, same-named entries are distinct by construction (IFP-22, Security Considerations).
...
- IFP-22 receivers interact with registries only through citations, the index, and entry fetches — all plain HTTPS GETs. No write interoperability exists or is intended; the PR path is human-governed.
...
A first instance is live at **`declarations.collectivesensecommons.org`** (Collective Sense Commons), seeded across the categories in Section 4 with deliberately varied, broadly non-confrontational entries — codes of conduct, professional ethics and pledges, political and social philosophies, and religious rules of life — chosen to demonstrate the range of scopes the format carries, and to make vivid that *other* registries may be as opinionated as their operators wish. Several of the traditions in scope have non-English sources — Greek, Arabic, Hebrew, Sanskrit, Pali, classical Chinese — and their entries will publish the source as the authentic text with courtesy translations alongside, exercising the Section 3.2 machinery from the first shelf. Others have unfree texts — professional codes under society copyright, standards incorporated by reference — and will enter as restatements, exercising Section 3.4 the same day. The two cases are deliberate: the seed shelf demonstrates every entry kind the format defines.

## References

- IFP-2 — Specification Style Guide · IFP-14 — Handling Hostile Content · IFP-20 — Hosted Mailbox Addressing (slug grammar) · IFP-22 — Declaration Posture
...
*This is IFP-21, Draft status. The frontmatter schema and category set will be refined by the first registry's build-out.*
