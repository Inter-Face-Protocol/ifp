...
**Why the declaration and the posture must be separate objects.** Google's code of conduct opened for years with the clause "Don't be evil," and then the company's relationship to it quietly changed. *The text never changed; the posture did.* You can only say that cleanly — or date it, or cite it — if the text and the stance toward it are separate, independently versioned things. Bake the stance into the text and the history becomes unrecoverable: there is nothing to diff but the document, and the document is innocent. This also shows why the registry's write-once rule earns its keep. "Google was `bound-by` this in 2010" stays verifiable after Google walks away, precisely **because Google does not control the copy**. In the real case the shift was legible only because someone happened to diff a web page; a posture would have been dated and citable from the start. Note also that the principal here is a corporation, not a person, and that the declaration is properly the code of conduct of which the motto is one clause — and since that code is copyrighted, shelving it would entail a restatement (IFP-21, Section 3.4) rather than a full text.
